  ...
  const apply = compiler => {
    ...
    const emit = (compilation, cb) => {
      ...
      patterns.forEach(pattern => {
        if (Array.isArray(pattern.from)) {
          pattern.from.forEach(fileName => {
            var filePattern = Object.assign({}, pattern);
            filePattern.from = fileName;
            tasks.push(
              Promise.resolve()
                .then(() => preProcessPattern(globalRef, filePattern))
                // Every source (from) is assumed to exist here
                .then(pattern1 => processPattern(globalRef, pattern1))
            );
          });
        } else {
          tasks.push(
